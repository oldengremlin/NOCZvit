...
### Додано
- **`claude.sentences`** — нова властивість конфігурації: задає максимальну кількість речень у резюме зміни (за замовчуванням 20); підставляється в промпт «до N речень»; парситься аналогічно до `claude.tokens`
