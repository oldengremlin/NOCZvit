...
- **Поле `inReplyTo` у `Incident`**: використовується як ключ пейрингу; для IMAP-інцидентів — значення `In-Reply-To:` заголовка; для Zabbix API-інцидентів — синтетичний ключ `"zabbix:host:clock"`; для OSM-інцидентів — порожній рядок
...
- **Кольорове кодування**: `row-start`/`row-end` CSS-класи залишаються лише для неспарених рядків; спарені рядки — без класу (нейтральний фон)
