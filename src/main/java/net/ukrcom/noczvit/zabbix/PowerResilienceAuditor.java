 ...
 * немає.
 *
 * <p>Мета — не «хто винен», а чи вузол протримався на резервному живленні не гірше за абонентські
...
public class PowerResilienceAuditor {
    ...
    private PowerResilienceResult auditOne(ZabbixProblem problem, Map<String, List<ZabbixProblem>> restartsByHost) {
        ...
        if (totalKnown > 0 && alreadyDown == totalKnown) {
            ...
            // Було: "Жоден з відомих портів не впав раніше за вузол." — нейтральне формулювання,
            // яке Claude самостійно домальовував до "коректної поведінки обладнання" — протилежного
            // до фактичного сенсу. Усі відомі порти були ще "up" на момент падіння самого вузла —
            // якщо це був епізод зникнення живлення, це означає, що ми не протрималися довше за
            // абонентів (у кращому разі — впали одночасно з ними).
