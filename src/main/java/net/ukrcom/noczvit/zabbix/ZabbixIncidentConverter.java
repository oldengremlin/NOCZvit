 ...
 * {@code ramos} — це вузол моніторингу датацентру, а не роутер); {@code uvpn},
 * {@code noc12593} та {@code ukrcom-gw} теж роутери, хоч і не починаються з 'r'
 * ({@code ukrcom-gw} — BGP-бордер AS12593, ядро мережі)</li>
 * </ul>
 */
@Slf4j
public class ZabbixIncidentConverter {
     ...
     * (datacenter monitoring unit, not a router despite the 'r'); uvpn/noc12593/ukrcom-gw →
     * маршрутизатор (routers whose hostname doesn't start with 'r'); a* → кондиціонер;
    ...
    private static String resolveDeviceWord(String host) {
        ...
        if (host.startsWith("uvpn") || host.startsWith("noc12593") || host.startsWith("ukrcom-gw")) return "маршрутизаторі ";
