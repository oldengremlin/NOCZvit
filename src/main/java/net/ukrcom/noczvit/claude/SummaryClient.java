...
public class SummaryClient {
    ...
    private String buildPrompt(List<Incident> incidents, LocalDateTime from, LocalDateTime to,
        ...
        return """
                ...
                - Якщо надано блок "АУДИТ РЕЗЕРВНОГО ЖИВЛЕННЯ": це ДОДАТКОВЕ ДЖЕРЕЛО КОНТЕКСТУ для відповідних host-down інцидентів (падіння вузла по ICMP), а НЕ самостійна тема. НЕ виділяй його в окремий абзац і НЕ перелічуй окремі порти чи інтерфейси — лише природно врахуй підсумковий висновок там, де в тексті вже йдеться про відповідний інцидент недоступності вузла (наприклад, чи вузол протримався на резервному живленні не гірше за абонентські порти).
