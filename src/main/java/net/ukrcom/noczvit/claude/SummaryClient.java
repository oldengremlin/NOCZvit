...
public class SummaryClient {
    ...
    private String buildPrompt(List<Incident> incidents, LocalDateTime from, LocalDateTime to) {
        ...
        return """
                ...
                Доменні знання для правильної оцінки подій:
                - "Routing Engine: High CPU utilization" (RE CPU) на маршрутизаторах Juniper — підвищене навантаження на Routing Engine. Модульна архітектура Juniper розділяє RE та TFEB/PFE: комутація пакетів та доступність мережі залишаються незачепленими. Це подія рівня "варто звернути увагу", а не аварія — описуй її нейтрально, без алармістського тону.
