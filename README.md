...
- **Службові й вільні порти ігноруються повністю** — не лише в переліках, а й у всіх лічильниках: порожній опис (`Interface 11()`) та явні позначки `(--free--)` / `(--unused--)` у будь-якому регістрі. Вільні порти шкідливі саме тут: вони завжди DOWN, тож завжди потрапляли б у «впали раніше вузла» і систематично зсували вердикт до сприятливого «резервне живлення протримало довше» без жодних підстав. Обгортка `--…--` обов'язкова, тож легітимний опис на кшталт `Interface 5(freedom cafe)` не втрачається. Кількість виключених показується окремим рядком
