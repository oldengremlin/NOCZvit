...
classDiagram
    ...
    class PowerResilienceResult["zabbix.PowerResilienceResult"] {
        ...
        +restartDetectedAt() Optional~Instant~
        +verdict() String
        +totalKnown() int
...
> ⚠️ **Секрети в командному рядку видно іншим користувачам системи.**
> Серед опцій є такі, що приймають секрети: `--zabbix-password`, `--mail-password`, `--claude-apikey`, `--account-mssql-password`, `--accequipment-mssql-password` і три `--snmp-community*` (SNMPv2c community — фактично пароль на читання всього обладнання).
>
> Аргументи будь-якого процесу в Linux доступні для читання всім користувачам системи через `ps aux` та `/proc/<pid>/cmdline`, а в інтерактивній оболонці команда ще й потрапляє в `~/.bash_history`. Отже, `--zabbix-password=secret` у команді — це розкриття пароля кожному, хто в цю мить має доступ до сервера.
>
> Опції залишені навмисно — для повноти дзеркалювання властивостей і для сценаріїв на кшталт разового запуску в контрольованому середовищі. Але **штатний спосіб зберігання секретів — файл `noczvit.properties`** з правами доступу лише для власника (`chmod 600`). Передавати секрети через CLI — свідомий вибір із відомим ризиком.
...
# (опціонально; потребує zabbix=true, а також incidents=true — аудит читає той самий
#  список подій Zabbix, який завантажується лише при увімкнених інцидентах)
resilienceaudit=false

# Технічні імена інтерфейсів (частина назви item до дужки з описом, напр. "wireguard2"
# у "Interface wireguard2(...)"), які виключаються з аудиту повністю — список через кому,
# регістронезалежний, порівняння за початком імені (startsWith). Порожньо за замовчуванням —
# нічого додатково не виключається. Призначення: віртуальні/тунельні інтерфейси на MikroTik
# (wireguard, sstp, l2tp, pptp тощо), які Zabbix бачить лише як текстове ім'я — тип інтерфейсу
# (ifType) у наявних даних не представлений, а додаткові SNMP-запити свідомо не робляться, тож
# це евристика за іменем, яку задає сам адміністратор.
resilienceaudit.ignoreinterfaceprefixes=wireguard,sstp
...
Усі рядки таблиці мають нейтральний сірий фон (аналогічно іншим таблицям звіту). `Sensor Error` у HTML сигналізує про можливе перезавантаження контролера RAMOS; у plain-text для Claude не включається.

Множина станів, що йдуть до Claude (`RamosTrapEvent.CLAUDE_STATES`), навмисно не збігається з рівнем «Критичний» — зростання й падіння температури в датацентрі не симетричні за ризиком:
- **`Low Warning`/`Low Critical` (падіння температури) виключені навіть на критичному рівні** — коли в датацентрі загалом тепло, зниження температури на одній ділянці рідко становить проблему, тож не варте уваги інженера в резюме зміни;
- **`High Warning`/`High Critical` (зростання температури), навпаки, варті уваги вже з рівня попередження** — це саме той бік, за яким варто стежити, доки не стало критично;
- **`Critical`/`Warning` без префіксу `High`/`Low`** — стосуються ненапрямлених датчиків (водяні, сухі контакти), де сам стан уже інформативний незалежно від напрямку.

До Claude не передається поле `sensorType`, щоб уникнути некоректного перекладу технічних назв типів датчиків.
...
Нумерація в колонці `№` тут **наскрізна** через усі кімнати: таблиці ділять один потік подій одного контролера, тож окремий відлік у кожній кімнаті лише заважав би зіставляти події між залами. Це відрізняється від секцій Emerson і аудиту резервного живлення, де кожна таблиця — окремий пристрій або окрема локація, і нумерація в кожній своя.

#### Порядок у HTML-звіті

```
[ Emerson trap секція       ]
[ RAMOS trap секція         ]  ← після Emerson
[ Аудит резервного живлення ]  ← опційно, одразу після RAMOS trap; перед боржниками
                                  (а на нічній зміні, де боржників немає, — перед температурою)
[ Боржники                  ]  ← лише на денній зміні
[ Температура SNMP          ]
[ Ramos SNMP                ]
[ PS-секція                 ]
```
...
Опційна секція (`resilienceaudit=true` / `--resilience-audit`, потребує `zabbix=true`), розміщується одразу після секції RAMOS trap — тобто перед боржниками на денній зміні, і перед температурою на нічній (де боржників немає). Акцентний колір — фіолетовий (`border-left:#7b1fa2`, текст `#4a148c`).

Відповідає на питання: чи наш вузол протримався на резервному живленні не гірше за абонентські порти. Для кожного вирішеного host-down інциденту (`Unavailable by ICMP ping`) `PowerResilienceAuditor` будує два знімки стану SNMP-моніторованих інтерфейсів хоста — точно на мить його падіння і точно на мить відновлення (`zabbix.Client.historyValueBefore`/`historyValueAfter`, `history.get` з `time_till`/`time_from` + `limit:1`, без жодного часового вікна). Період, поки вузол сам був недоступний, принципово не описується — Zabbix фізично не опитує інтерфейси хоста, поки той сам down.

- Порти, що вже впали до падіння вузла, і порти, що ще працювали — окремі лічильники; з тих, що ще працювали, окремо рахуються ті, що фіксувалися як активні на момент відновлення вузла (це не «піднялись раніше вузла» — за двома знімками не видно, чи порт взагалі падав), і ті, що лишились недоступні й після його відновлення (друга частина речення опускається, якщо таких немає)
- М'який вердикт («ймовірно протримали довше» / «ймовірно, протримали не довше за абонентів») з'являється лише на двох однозначних краях — усі відомі порти впали раніше вузла, або жоден не впав раніше (усі лишались активними на момент падіння вузла); лише факт, без рекомендацій — в іншому разі рядок узагалі порожній, висновок за інженером NOC
- **Підтвердження перезавантаження через тригер Zabbix «`{host} has been restarted`».** Сам цей тригер («Template Module Generic SNMPv2») теж заснований на `{HOST}:system.uptime.last()<10m` — тобто на тому самому лічильнику, що ризикує переповнюватись, і ізольовано не надійніший за голе зменшення uptime. Надійним його робить те, що аудит бере лише подію, яка настала одразу після підтвердженого ICMP-відновлення того самого хоста: щоб пояснити такий збіг переповненням лічильника, воно мало б статися точно в момент реального ICMP-обриву й відновлення — украй малоймовірно. Тому показується завжди, коли знайдений — незалежно від вердикту по портах — і заміняє собою слабший, ізольований uptime-факт нижче, коли обидва присутні. Прив'язка без часового вікна: береться найближча подія цього ж хоста з часом не раніше відновлення хоста (`tUp`) — раніше Zabbix фізично не міг би виявити перезавантаження, бо ще не опитував хост
- Зменшення лічильника `system.uptime` (якщо доступний item, і «has been restarted» НЕ знайдено) показується лише в цій неоднозначній середині — ніколи поруч із чітким вердиктом чи підтвердженим перезавантаженням — і завжди з застереженням, що 32-бітний лічильник може переповнюватись і без реального перезавантаження
- **Два різні «немає даних» рахуються окремо.** `noDataAtFall` — немає знімка на момент падіння, порт не входить у знаменник узагалі. `noDataAtRecovery` — порт **уже** порахований у «ще працювали», бракує лише другого знімка. Раніше обидва йшли в один лічильник під одним підписом, через що речення «З тих, що ще працювали…» не сходилось із загальним числом
- **Службові й вільні порти ігноруються повністю** — не лише в переліках, а й у всіх лічильниках: порожній опис (`Interface 11()`) та явні позначки `(--free--)` / `(--unused--)` у будь-якому регістрі. Вільні порти шкідливі саме тут: вони завжди DOWN, тож завжди потрапляли б у «впали раніше вузла» і систематично зсували вердикт до сприятливого «резервне живлення протримало довше» без жодних підстав. Обгортка `--…--` обов'язкова, тож легітимний опис на кшталт `Interface 5(freedom cafe)` не втрачається. Розпізнавання — не regex з дедалі складнішими класами символів, а обрізання типової «людської» декорації (пробіли, лапки, беклеші — `(\"--free--\")` трапляється в реальних даних після ручного редагування опису в Zabbix) з точним порівнянням залишку з `--free--`/`--unused--`: так закривається клас випадків, які плоский regex не покриває, і водночас не з'являється хибних спрацювань, коли маркер — лише частина довшого змістовного опису (`(-- unused --, чекає на міграцію)` не ігнорується). Кількість виключених показується окремим рядком
- **Віртуальні/тунельні інтерфейси теж ігноруються повністю — за префіксом технічного імені** (`resilienceaudit.ignoreinterfaceprefixes`, порожньо за замовчуванням). UVPN-вузли на MikroTik (RB3011UiAS) мають, крім фізичних портів, інтерфейси на кшталт `wireguard`/`sstp` — Zabbix бачить лише текстове ім'я (`ifDescr`/`ifName`), тип (`ifType`), яким на MikroTik легко відрізнити фізичний порт, у наявних даних не представлений, а додаткові SNMP-запити поза Zabbix свідомо не робляться. Тому це евристика за іменем (`startsWith`, регістронезалежно), яку задає сам адміністратор — той самий лічильник `ignoredPorts`, що й для службових/вільних портів
- `item.get` запитує лише увімкнені items (`filter: {status: 0}`) — вимкнені свіжої історії не мають і лише роздували б лічильник «немає даних»
...
- **Групування по локації, таблицею — як і решта секцій звіту.** Один винос часто кладе кілька SNMP-моніторованих вузлів одразу (наприклад `ssks-2`/`ssks-4`/`ssks-5` на «Бандери 8 (СКС)»), тож локація виводиться `<h3>` один раз, а під нею — таблиця з одним рядком на вузол. Колонки — `№`, `Обладнання`, `Початок`, `Закінчення`, `Тривалість` (усі названі так само, як у таблиці інцидентів) і `Результат аудиту`, куди йде весь розбір по портах. Нумерація своя в кожній таблиці — як у секції Emerson, де кожен пристрій має власну таблицю; тут «своя таблиця» на кожну локацію. Остання комірка навмисно велика — це ціна однотипності з рештою звіту
...
Паралелізм — на двох рівнях, обидва через `ConcurrentPoll` (той самий механізм, що й у `snmp.Client`): інциденти між собою (`MAX_CONCURRENT_AUDITS = 5`) і порти всередині одного інциденту (`MAX_CONCURRENT_PORT_PROBES = 4`). Реальна межа навантаження на Zabbix — добуток, тобто 20 одночасних запитів, того ж порядку, що й SNMP-опитування (10). Без другого рівня хост на 40 портів давав до 80 **послідовних** HTTP-обходів на один інцидент. `ConcurrentPoll` зберігає порядок вхідного списку, тож переліки в звіті від паралелізації не змінюються, а лічильники агрегуються вже в потоці-викликачі й між задачами не діляться.

Списки items (`getInterfaceItems`, `getUptimeItem`) кешуються по хосту: один вузол нерідко падає кілька разів за зміну, і кожен такий інцидент аудитується окремо.
...
| Уся фаза паралельної ініціалізації | `orTimeout(10 хв)` на `allOf(...)` + `shutdownNow()` на аварійному шляху |

**Чому `orTimeout` замало.** Він не скасовує задачу — лише завершує обгортковий future винятком, а сама зависла задача працює далі. Тому виконавець ініціалізації навмисно **не** в `try-with-resources`: штатний `close()` робить `shutdown()` + `awaitTermination(1 доба)` **без** переривання, і на аварійному шляху cron-запуск завис би на добу — рівно те, від чого таймаут мав захищати. Успішний шлях завершується `close()`, аварійний — `shutdownNow()`, який задачі перериває.

`zabbixFuture` перелічений у `allOf(...)` явно. Транзитивно його покривають лише `zabbixProblemsFuture` і `resilienceFuture`, а обидва вимикаються своїми прапорцями — при `--zabbix --no-incidents` він інакше лишався б і поза таймаутом, і поза централізованою обробкою помилок.

**Важливо при редагуванні кешів у `zabbix/Client`:** `resolveHostId`/`resolveGraphId`/`getInterfaceItems`/`getUptimeItem` навмисно написані як `get` → HTTP → `putIfAbsent`, а **не** через `computeIfAbsent`. Останній виконує mapping-функцію під `synchronized` на вузлі корзини, тобто монітор утримувався б увесь HTTP round-trip (до 30 с), блокуючи інші потоки з ключами тієї ж корзини. На Java 21 (`<release>21</release>`) блокування всередині `synchronized` ще й **пришпилює віртуальний потік до несучого** — JEP 491 прибрав це лише в JDK 24, — тож обмежений fan-out перетворювався б на стільки ж заблокованих платформних потоків. Ціна поточного варіанту — можливий повторний запит для того самого ключа, що ідемпотентний.
...
### Ширина колонок у таблицях

Колонки зі сталим за довжиною вмістом позначені класом `nw` (`th.nw,td.nw{white-space:nowrap}`): `Початок`, `Закінчення`, `Тривалість`, `Обладнання`, `Час` і `Стан`. Описові колонки (`Інцидент`, `Подія`, `Результат аудиту`, `Назва датчика`, `Тип датчика`) навмисно лишені переносними — саме вони й мають забирати залишок ширини.

Це дає вирівнювання **між** секціями безкоштовно: дата у звіті завжди має вигляд `05 серп 2026 13:37:15`, тож щойно їй заборонено переноситись, браузер дає колонці рівно ту ширину, що потрібна цьому рядку — а вона однакова в усіх таблицях. Раніше Emerson пінив `width:155px`/`100px`, а інциденти й аудит не пінили нічого, через що ті самі колонки в різних секціях розходилися. Жорсткі пікселі прибрано: у поштовому HTML вони або обрізають вміст на вузьких екранах, або лишають порожнечу, і не переживають зміни формату дати чи шрифта. Єдина фіксована ширина, що лишилась, — `width:30px` на колонці `№`.
...
| **Заголовок `Date:` IMAP-листа** (колонки «Початок»/«Закінчення») | `IncidentSectionBuilder` |
...
Окремо про заголовок `Date:`: він недовірений нарівні з темою листа. Строгий парсер на сміттєвому хвості падає, але `MailMessageSupport` тут-таки робить fallback на `getSentDate()` — лист приймається, а `dateStr` зберігається сирим. Санітизація в `convertMonthNumToMnemo` тут не допоміжна: регулярка зони `\s*\+\d{4}$` прив'язана до кінця рядка, тож будь-який хвіст після зони і виживає, і блокує її зняття. Єдиний захист — екранування на виводі.

`Config` позначено `@ToString(exclude = …)` для всіх паролів, API-ключа, **SNMP community strings** та поля `properties` — щоб випадковий `log.debug("config={}", config)` не вивалив секрети в лог. Community для SNMPv2c — фактично пароль на читання всього обладнання, тож виключений нарівні з рештою секретів.

Дані, що йдуть у промпт Claude (перелік інцидентів і блок подій датацентру), обгорнуті маркерами `=== ПОЧАТОК … ===` / `=== КІНЕЦЬ … ===`, а промпт містить явну вказівку трактувати вміст між ними як дані, а не як інструкції. Це важливо, бо підсумок зберігається в SQLite і підмішується в промпт **наступної** зміни — без ізоляції один отруєний лист впливав би на ланцюжок звітів.

Самих маркерів недостатньо, якщо дані можуть їх підробити: тема листа чи назва датчика, що містить літерально `=== КІНЕЦЬ ДАНИХ ===`, обірвала б блок, і решта тексту прочиталась би як інструкція. Тому кожне зовнішнє значення проходить через `SummaryClient.forPrompt()`, який замінює послідовності з трьох і більше `=` на `---`. Санітизація застосована на **листках** — там, де значення вперше потрапляє в текст для Claude (поля інциденту в `SummaryClient`, hostname/IP/опис у `EmersonTrapSection`, назва датчика в `RamosTrapSection`, host/локація в `PowerResilienceSection`), а також до резюме попередньої зміни з SQLite — щоб отруєний звіт не проніс маркер у наступний. Уже зібрані plain-text блоки через фільтр не проганяються: їхні власні маркери легітимні.
...
│   │   ├── OsmIncidentParser.java — OSM/SDH (Power, STM-N); Trap value → точний час події (пізніший за алерт — обрізається; уточнення «який відбувся» лише коли раніший щонайменше на 5 хв)
