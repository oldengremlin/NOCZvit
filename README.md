...
# claude.sentences=20     ← максимальна кількість речень у резюме (за замовчуванням 20)
...
- Мова резюме: **системне повідомлення** вимагає відповіді виключно **українською** (без русизмів, офіційний стиль); `fixRussianisms()` замінює відомі слова-русизми («события/собитія» тощо) у відповіді; `warnIfRussian()` виявляє символи ы/ъ/э/ё і виводить попередження у лог
...
Параметр `claude.sentences` (за замовчуванням 20) задає максимальну кількість речень у резюме — підставляється в інструкцію Claude «до N речень».
