...
- **Групування по локації.** Один винос часто кладе кілька SNMP-моніторованих вузлів одразу (наприклад `ssks-2`/`ssks-4`/`ssks-5` на «Бандери 8 (СКС)») — секція виводить `<h3>` локацію один раз, а кожен вузол під нею — окремим `<h4 class="resilience-host-sub">` з такою самою «підсвіткою рядка»; текст під кожним вузлом обгорнутий в `<div class="resilience-host-body">` і зсунутий вправо (`margin-left:24px`) — цього кадрового зсуву вже достатньо, щоб вкладеність читалась з першого погляду, без додаткових візуальних маркерів. Коли локація не розпізнана (дорівнює самому hostname), зайвий рівень вкладеності не додається
