...
- **Групування по локації, таблицею — як і решта секцій звіту.** Один винос часто кладе кілька SNMP-моніторованих вузлів одразу (наприклад `ssks-2`/`ssks-4`/`ssks-5` на «Бандери 8 (СКС)»), тож локація виводиться `<h3>` один раз, а під нею — таблиця з одним рядком на вузол. Колонки: `Вузол`, `Початок`, `Закінчення`, `Тривалість` (три останні названі так само, як у таблиці інцидентів) і `Результат аудиту`, куди йде весь розбір по портах. Остання комірка навмисно велика — це ціна однотипності з рештою звіту
