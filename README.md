...
- **Пряме підтвердження перезавантаження через тригер Zabbix «`{host} has been restarted`».** Це вже готовий висновок самого Zabbix, а не наша інтерпретація сирих даних, тож показується завжди, коли знайдений — незалежно від вердикту по портах — і заміняє собою слабший uptime-факт нижче, коли обидва присутні. Прив'язка без часового вікна: береться найближча подія цього ж хоста з часом не раніше відновлення хоста (`tUp`) — раніше Zabbix фізично не міг би виявити перезавантаження, бо ще не опитував хост
